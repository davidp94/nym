...
# TODO: put all the repeating expressions in loops...
...
build_servers:
	@if ! [ -f build/issuers/issuer1/config.toml ]; then \
		...
		cp daemon/server/sampleKeys/coconutkeys/secret0-n=3-t=2.pem build/issuers/issuer1/coconutkeys/ ;\
		cp daemon/server/sampleKeys/coconutkeys/verification0-n=3-t=2.pem build/issuers/issuer1/coconutkeys/ ;\
		cp daemon/server/sampleKeys/coconutkeys/secret1-n=3-t=2.pem build/issuers/issuer2/coconutkeys/ ;\
...
localnet-build:
	@if ! [ -f build/nodes/node0/config/genesis.json ]; then  \
		mkdir -p build/nodes/node0/config; \
		...
		sed -i -e "s/$(APP_STATE_ORIGINAL)/$(APP_STATE_REPLACEMENT)/g" build/nodes/node2/config/genesis.json ; \
		sed -i -e "s/$(APP_STATE_ORIGINAL)/$(APP_STATE_REPLACEMENT)/g" build/nodes/node3/config/genesis.json ; \
	fi
	make build_abci
	make build_servers
...
build_abci:
	@if ! [ -f build/abci0/nymDB.db ]; then  \
		mkdir -p build/abcis/abci0 ; \
		mkdir -p build/abcis/abci1 ; \
		mkdir -p build/abcis/abci2 ; \
		mkdir -p build/abcis/abci3 ; \
	fi
	docker build --rm -t nym/nymabci -f ./DOCKER/nym_abci/Dockerfile . 
